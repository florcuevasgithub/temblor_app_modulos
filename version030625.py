...
Original file is located at
    https://colab.research.google.com/drive/1YY4Eutvn3axvIjmwxTcypbAhhT86cErG
"""
...
def analizar_temblor_por_ventanas_resultante(df, fs=100, ventana_seg=2):
            ...
            for i in range(len(acc)):
              q = Q[i] # Cuaternión de orientación para el instante actual
              acc_measured = acc[i] # Aceleración total medida por el sensor en su propio marco
              R_W_B = q_to_matrix(q)
            ...
            for i in range(num_ventanas):
              ...
              if len(segmento) < tamaño_ventana:
                continue
              segmento = segmento - np.mean(segmento) # Centrar el segmento para el análisis espectral

              # Análisis de frecuencia usando el método de Welch
              f, Pxx = welch(segmento, fs=fs, nperseg=tamaño_ventana)
              freq_dominante = f[np.argmax(Pxx)] # Frecuencia con la mayor potencia

              # Métricas de temblor
              varianza = np.var(segmento)
              rms = np.sqrt(np.mean(segmento**2))
              amp_g = (np.max(segmento) - np.min(segmento))/2 # Amplitud pico m/s2
            ...
            # Ahora calculás el promedio como antes:
...
if opcion == "1️⃣ Análisis de una medición":
        ...
        if st.button("Iniciar análisis"):
            ...
            ventanas_por_test = []  # Lista para guardar cada df_ventanas + nombre del test
